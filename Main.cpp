...
#include <memory>
//Лямбда в лямбде
const auto x = [](int a){//1) в первую очередь вызывается x и копируется а
	const auto y = [a](int b){//3) перходим в y и выполняем его
		return a + b + 5;
	}
	return y(10);//2) перходим ретерн и пробрасываем 10 в b 
}
//int main (){
//	x(6);//4) возвращаем значение 21 в вызов в функции
//	return 0;
//}
